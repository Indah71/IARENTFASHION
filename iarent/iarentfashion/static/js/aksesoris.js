...
class AksesorisApp {
    ...
    init() {
        ...
        this.loadWishlist();
        this.loadCart();
        console.log('Aksesoris App initialized');
    }
    ...
    sendAddToCartRequest(productId, product, event) {
        ...
        .then(data => {
            if (data && data.success) {
                this.handleAddToCartSuccess(product, event, data.cart_count);
            } else if (data) {
                alert('❌ ' + data.message);
            }
        })
    ...
    handleAddToCartSuccess(product, event, cartCount) {
                    ...
                    <div class="cart-item">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-end">
                                <button class="remove-item" onclick="aksesorisApp.removeFromCartServer(${item.id})" title="Hapus dari keranjang">
    ...
    addToWishlistLocal(productId) {
        ...
        if (existingItemIndex === -1) {
            ...
            const productToAdd = {
                id: productId,
                name: product.name,
                category: product.category || 'Aksesoris',
                price: product.price,
                image: product.image
            };
                    ...
                    wishlistHTML += `
                        <div class="d-flex justify-content-between align-items-center mb-2 p-2 border-bottom">
                            ...
                            <div class="text-end">
                                ...
                                <button class="btn btn-sm btn-outline-danger ms-2" onclick="aksesorisApp.removeFromWishlist(${item.id})">
                    ...
                    cartHTML += `
                        <div class="d-flex justify-content-between align-items-center mb-2 p-2 border-bottom">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <div class="d-flex align-items-center mt-1">
                                        <button class="btn btn-sm btn-outline-secondary p-0" style="width: 20px; height: 20px;" onclick="aksesorisApp.updateCartQuantity(${item.id}, -1)">-</button>
                            ...
                            <div class="text-end">
                                ...
                                <button class="btn btn-sm btn-outline-danger ms-2" onclick="aksesorisApp.removeFromCartLocal(${item.id})">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    window.aksesorisApp = new AksesorisApp();
});

// Global functions untuk HTML onclick attributes
function goToDetail(productId) {
    window.aksesorisApp.goToDetail(productId);
}

function addToCart(productId) {
    window.aksesorisApp.addToCart(productId);
}

function addToWishlist(productId) {
    window.aksesorisApp.addToWishlist(productId);
}

function addToWishlistLocal(productId) {
    window.aksesorisApp.addToWishlistLocal(productId);
}

function addToCartLocal(productId) {
    window.aksesorisApp.addToCartLocal(productId);
}

function continueShopping() {
    window.aksesorisApp.continueShopping();
}

function checkout() {
    window.aksesorisApp.checkout();
}

function handleSewaClick() {
    window.aksesorisApp.handleSewaClick();
}

function showCart() {
    window.aksesorisApp.showCart();
}

function removeFromCart(productId) {
    window.aksesorisApp.removeFromCartLocal(productId);
}

function updateCartCount() {
    window.aksesorisApp.updateCartCount();
}